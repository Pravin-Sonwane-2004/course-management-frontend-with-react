  ...
  const fetchInstances = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        const formattedInstances = response.data.map(instance => ({
          ...instance,
          courseId: instance.course?.courseId || instance.courseId,
          courseName: instance.course?.name || 'Loading...'
        }));
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {instances.map((instance) => (
                  <tr key={instance.instanceId}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">{instance.course?.name || 'Loading...'}</td>
